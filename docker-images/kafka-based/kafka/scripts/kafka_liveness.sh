...
if [ "$USE_KRAFT" == "true" ]; then
  ...
  # We expect that either the broker is ready and listening on 9096 (replication port)
  # or it has a ZK session
  if [ -f /var/opt/kafka/kafka-ready ] ; then
    rm -f /var/opt/kafka/zk-connected 2&> /dev/null
    # Test listening on replication port 9096
    netstat -lnt | grep -Eq 'tcp6?[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[^ ]+:9096.*LISTEN[[:space:]]*'
  else
